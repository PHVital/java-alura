...
public class ConsultaService {
    ...
    public Page<DadosListagemConsulta> listar(Pageable paginacao, Usuario logado) {
        ...
        return repository.buscarConsultas(logado.getId(), paginacao).map(DadosListagemConsulta::new);
    }
    ...
    public void cadastrar(DadosAgendamentoConsulta dados, Usuario logado) {
        ...
        if (logado.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_PACIENTE")) && !pacienteConsulta.getId().equals(logado.getId()))
    ...
    @PreAuthorize("hasRole('ATENDENTE') or (hasRole('PACIENTE') and @consultaRepository.findById(#id).get().paciente.id == principal.id)")
    ...
    @PreAuthorize("hasRole('ATENDENTE') or (hasRole('PACIENTE') and @consultaRepository.findById(#id).get().paciente.id == principal.id) or (hasRole('MEDICO') and #id == principal.id)")
    @Transactional
